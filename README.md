...
硬盘里的 docx、ppt、网页、表格,多半存了就忘。<br>
z-wiki 把它们编译成统一的 markdown 知识库,agent 替你读、写、编辑、搜索、串联——<br>
零散的文档长成一座能翻阅、能追问的书架。数据在本机,本地优先。

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Release](https://img.shields.io/github/v/release/ember-hearted/z-wiki)](https://github.com/ember-hearted/z-wiki/releases)
[![Platform](https://img.shields.io/badge/Platform-Windows%20%C2%B7%20macOS%20%C2%B7%20Linux-lightgrey)](https://github.com/ember-hearted/z-wiki/releases)

[项目介绍页](https://ember-hearted.github.io/z-wiki/) · [下载 Release](https://github.com/ember-hearted/z-wiki/releases) · [问题反馈](https://github.com/ember-hearted/z-wiki/issues)
...
- **上传自动编译** —— 拖拽上传 docx、xlsx、pptx、odt、epub、html、csv 等文档,内置 pandoc 转 markdown,agent 自动编译进知识库(原文归档 `raw/`)。
...
- **增量自动更新** —— 自建三档增量分发(完整包 / 应用包 / 代码包),按版本差异自动选最小的包。

![z-wiki 档案索引](docs/img/z-wiki-archive-index.png)

## 安装

从 [Releases](https://github.com/ember-hearted/z-wiki/releases) 拿对应平台的包:
...
装好后先到「设置」填 LLM 配置(Base URL / 模型 / API Key),agent 才能调用。

<details>
<summary><strong>首次启动较慢,属正常现象</strong></summary>

首次启动需等几十秒到 1 分钟,不要以为卡死打不开:

- 首次需把内置的 pandoc(约 180MB)从安装包复制到用户数据目录(`copyFileSync` 同步,阻塞主进程)。
- 旧 Windows(见「旧 Windows 兼容」)禁 GPU 后走软件渲染,初始化也慢。
...
<details>
<summary><strong>旧 Windows 兼容</strong></summary>
...
<details>
<summary><strong>数据位置</strong></summary>
...
- **自动更新覆盖场景**:代码包 / 应用包档自动下载、重启生效;跨 baseline 升级(Electron / 工具二进制变)时 mac / win 需下载完整包手动重装。Linux 无增量,每次更新自动下载完整 AppImage(约 180MB)原地替换。
