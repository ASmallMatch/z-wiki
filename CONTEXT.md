...
- **Draft(浅)** -- 净纸白浅色主题。中性暖白纸底 + 明快墨蓝 accent(ADR-0022);陶土橙留书架色板偶发暖点缀(ADR-0006 材质正色传承)。accent 与 Archive 同源蓝系(墨蓝↔靛青),切换时不跳变。
- **书架随主题** -- 首页 3D 书架的展台底色与书配色随主题切换(Archive 深舞台 / Draft 浅展台)。两套书皮纹理在主题切换时换 texture,不重建 3D 场景。
- **中性载体** -- 书皮底色:主题自适应中性色(Archive 深灰 / Draft 纸白),零色相、不随书变化。supersedes 彩色书皮满铺混色(ADR-0020 D1,见 ADR-0023)。
- **封面画** -- 每本书书皮上按书名 hash 程序化生成的抽象构图,是书皮彩色的唯一来源(载体本身不带色)。彩色取自该书 hash 分配的 accent 色板(6 色色相族:靛蓝/青碧/松绿/陶土/茜红/堇紫),Archive 高饱和 / Draft 柔粉彩,同一本书两主题同构图同色相昼夜呼应(ADR-0022 D3)。
- **构图原型** -- 封面画的几种固定构图套路(几何色块/斜切色带/渐变大字卷号等),按书名 hash 选原型并定参数,一排书每本构图不同。每种原型给书脊标题预留低对比的**标题安静区**;纸边与卷号标是保留的身份元素,胶带/档案条降为原型的可选部件。
- **主题开关** —— header 设置按钮右侧的 pill 滑动开关(月左日右),无文字、纯图标,控制明暗切换。a11y 对齐项目自定义控件基准(`role="switch"`+键盘)。
- **分级柔化** —— 全站圆角语言分两级:容器级(按钮/卡片/气泡/抽屉)柔化,小件级(chip/徽章/代码块/小标签)保持锋利(书卷号标签感在小件延续)。全站 finish 对齐 docs/index.html 展厅语言(渐变主按钮/辉光阴影/hover 微抬升);辉光为 Archive 专属,Draft 依 ADR-0013 降级为描边。
- **kicker** —— mono + 宽字距的小标签语言,用于对话界面的署名与小标签(Fairy 标签/思考/工具/角标/状态行);❦ 花朵署名标为保留的签名元素。

## 书架槽位(layer2 可视层)

首页 3D 书架的圆柱槽位编址与交互术语。仅领域语言,实现细节见 ADR-0015。
...
- **轨道球** -- 聚焦槽抽出本的自由翻转交互态:鼠标位置驱动该单一本书三轴旋转(中键点中心本/空格进出)。与拖拽转滑轨区分:拖拽转的是整排书的槽位,轨道球只转聚焦的单一本,槽位不动。

_Avoid_: 把虚拟位叫"假书/占位书"(它不渲染成书);把 realSlots 叫"可见槽"(虚拟位也在可见窗口,只是空);把轨道球叫"拖拽/旋转书"(拖拽是整排滑轨,轨道球是单本三轴翻转,两者不同态)。
