...
// 造 bundle 目录:rg/fd 假二进制(内容标记版本便于区分)+ version.json。
...
test('needsRelayout: 目标无版本文件 → true(首次需铺放)', async () => {
  const { paths, cleanup } = await makePaths({ rg: '14.1.0', fd: '10.1.0' })
  try {
...
test('needsRelayout: 版本不一致 → true(需升级)', async () => {
  ...
  try {
    layoutToolBins(paths.toolBinsPath, paths.binDir)
    // bundle 升级 rg 版本,目标仍是旧版 → 需重铺
    await makeBundle(paths.toolBinsPath, { rg: '14.2.0', fd: '10.1.0' })
    assert.equal(needsRelayout(paths.toolBinsPath, paths.binDir), true)
  } finally {
    await cleanup()
  }
...
test('needsRelayout: 版本一致但二进制缺失 → true(需重铺)', async () => {
  ...
  try {
    layoutToolBins(paths.toolBinsPath, paths.binDir)
    await fs.rm(path.join(paths.binDir, RG))
    assert.equal(needsRelayout(paths.toolBinsPath, paths.binDir), true)
  } finally {
    await cleanup()
  }
...
test('layoutToolBins: 复制 rg/fd + 写 .version.json + unix 二进制可执行', async () => {
  const { paths, cleanup } = await makePaths({ rg: '14.1.0', fd: '10.1.0' })
  try {
    layoutToolBins(paths.toolBinsPath, paths.binDir)
    ...
    // unix 二进制必须可执行(pi spawn 需要 +x)
    if (process.platform !== 'win32') {
      const mode = statSync(path.join(paths.binDir, RG)).mode
      assert.ok(mode & 0o755, 'rg 应有可执行权限')
    }
  } finally {
    await cleanup()
  }
...
test('ensureToolBins: 首次铺放 → 二次(版本一致)跳过不重写', async () => {
  ...
  try {
    ...
    assert.ok(existsSync(path.join(paths.binDir, RG)))
    // 记录 mtime,二次启动不应重写
    const mtimeBefore = statSync(path.join(paths.binDir, RG)).mtimeMs
    await new Promise((r) => setTimeout(r, 20))
    ensureToolBins(paths)
    const mtimeAfter = statSync(path.join(paths.binDir, RG)).mtimeMs
...
test('ensureToolBins: 版本升级 → 重新铺放(内容更新)', async () => {
  ...
  try {
    ensureToolBins(paths)
    const before = await fs.readFile(path.join(paths.binDir, RG), 'utf-8')
    assert.match(before, /rg-14\.1\.0/)
    // bundle 升级
    await makeBundle(paths.toolBinsPath, { rg: '14.2.0', fd: '10.1.0' })
    ensureToolBins(paths)
    const after = await fs.readFile(path.join(paths.binDir, RG), 'utf-8')
    assert.match(after, /rg-14\.2\.0/, '升级后二进制内容已更新')
  } finally {
    await cleanup()
  }
})
