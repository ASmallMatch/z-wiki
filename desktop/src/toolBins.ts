// toolBins.ts — rg/fd 二进制预打进(ADR-0003 D8)。
// bundle 内 resources/bin/<platform>-<arch>/ 含 rg/fd + version.json;
// 启动时检测 <binDir>/.version.json 是否一致 → 不一致或缺失则重新铺放(支持升级)。
// pi 的 getToolPath 优先查 <agentDir>/bin(= binDir),PI_OFFLINE=1 禁下载,故必走预打进。
import {
  chmodSync,
  copyFileSync,
  existsSync,
...
// pi 工具集需预打进的两个二进制(AGENT_TOOLS 中 spawn 外部进程的只有 grep→rg / find→fd)。
  ...
  if (!bundleVersion) {
    throw new Error(
      `bundle 缺少版本文件:${path.join(toolBinsPath, BUNDLE_VERSION_FILE)}\n` +
        '请运行 tsx scripts/fetch-tool-bins.ts 预打进 rg/fd 二进制。',
    )
  }
  const targetVersion = readVersion(path.join(binDir, TARGET_VERSION_FILE))
  if (!targetVersion) return true
  if (targetVersion.rg !== bundleVersion.rg || targetVersion.fd !== bundleVersion.fd) return true
  return !binariesPresent(binDir)
}

/** 从 bundle 复制 rg/fd 到 binDir + 写目标版本文件 + chmod +x(unix)。 */
