...
interface BookThemeColors {
  ...
  accents: string[] // 书身份色板(按书名 hash 分配;胶带/边框/卷号标 + 书皮派生源)
  dirLightIntensity: number // 主光强度
  rimLightColor: number // rim 点光颜色
}

// Archive:深色舞台 + 鲜明档案色板;书皮 = accent 混展台深底 62%(深彩书皮)
...
function coverBase(accent: string, theme: Theme): string {
  return theme === 'draft'
    ...
    : mixColor(accent, DARK_BASE, 0.62)
}
...
function makeCoverTexture(data: {
  ...
  ctx.fillStyle = contrastColor(data.accent)
  ...
  ctx.strokeStyle = textColor === '#fff' ? 'rgba(255,255,255,0.12)' : 'rgba(28,25,23,0.1)'
  ctx.lineWidth = 2
  ...
  // 浅色书皮(Draft 粉彩)上 accent 阴影会烟熏,去阴影;深色书皮保留墨水辉光
  ...
  // 底部档案条:accent 暗化 30%,与胶带/边框/卷号同族(整书收敛到 accent 三色体系)
  ctx.fillStyle = mixColor(data.accent, '#000000', 0.3)
...
function makeSpineTexture(data: {
  ...
  ctx.strokeStyle = textColor === '#fff' ? 'rgba(255,255,255,0.15)' : 'rgba(28,25,23,0.18)'
  ctx.lineWidth = 2
  ctx.beginPath()
  ctx.moveTo(16, 0)
  ctx.lineTo(16, 700)
  ...
  grd.addColorStop(0.5, textColor === '#fff' ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.22)')
  ...
  useEffect(() => {
    ...
    const makeSkinPool = (colors: BookThemeColors, theme: Theme): BookSkin[] =>
      sorted.map((page) => {
        ...
        const dark = coverBase(accentHex, theme) // 彩色书皮:accent 派生底色(ADR-0020)
        return {
