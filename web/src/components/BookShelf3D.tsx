...
export default function BookShelf3D({ pages, onBookClick, onIntroDone, theme }: BookShelf3DProps) {
  ...
  useEffect(() => {
    ...
    function snapToNearest() {
      ...
      const target = snapTarget(rot.val, effStep, half, slots, virtual)
      snapping = true
      gsap.to(rot, {
        val: target,
        duration: 0.5,
    ...
    function enterOrbit() {
      ...
      const effStep = ANGLE_STEP * (1 + SPREAD_MAX * spreadP.val)
      rot.val = orbitAlignTarget(rot.val, effStep, slots)
      orbiting = true
    }
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      if (slots <= 1) {
        // 单本书橡皮筋渐近线（soloElasticRot）：跟手范围内 1:1，超出渐近趋向 ±limit
        rot.val = soloElasticRot(raw, soloMaxRot)
      } else if (!virtual) {
        const effStep = ANGLE_STEP * (1 + SPREAD_MAX * spreadP.val)
        rot.val = clampRot(raw, effStep, half)
      } else {
        rot.val = raw
      }
