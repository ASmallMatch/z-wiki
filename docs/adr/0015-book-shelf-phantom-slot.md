# ADR-0015: N≤3 补虚拟位凑奇数 slots,保留 slot0 体系

- 状态:accepted
- 日期:2026-07-16
- 范围:layer2(web)`BookShelf3D.tsx` slots 计算与 currentSlot 量化。N≤3 补虚拟满窗(N=1,2 -> slots=3;N=3 -> slots=5),N≥4 原 reflow 不变。
...
N=2 时 min=2、奇偶化砍成 1,只建 1 个书对象,第二本丢失(用户 bug:上传 2 本只见 1 本)。N=1 走 `slots<=1` 单本特殊路径(独立橡皮筋动画)。根因:奇数化与 N=1,2 冲突。N=3 满窗不崩但顺序乱(slotIndex=−1 映射 dataIndex 2 最旧,向右拖第一->第三跳过第二);N≥4 `slots=N−1` 奇 + virtual reflow 遍历(小窗口,顺序乱但滑动遍历不显)。

## 决策

### D1:N≤3 补虚拟满窗 + realSlots 正侧顺序对

`computeShelfSlots`:
- N≤3:`slots = max(3, 2N−1)`。N=1,2 -> 3;N=3 -> 5。realSlots 正侧 [0..N−1],向左拖 第一->第二->第三 顺序对(slot0 钉中心 + mod 编址,正侧优先占位使负侧 mod 重复成虚拟位)。
- N≥4:`slots = min(N, 17, max(N−1,3))` 奇数化(原 reflow 设计,滑动遍历)。

N=3 补到 5 而非 3:slots=3 时 realSlots=[−1,0,1],slotIndex=−1 映射 dataIndex 2(最旧),向右拖第一->第三 跳过第二(顺序乱);补到 5 realSlots=[0,1,2] 正侧,向左拖顺序对,代价是 2 个虚拟空位(负侧 −1,−2)。

### D2:虚拟位空槽(mod 重复位),不建对象

N≤3 补虚拟时,slotIndex mod N 重复位不建 mesh、不进 `allSlots`/`slotMap`(`computeRealSlots` 去重,正侧优先)。N=1:虚拟 ±1、真书 {0};N=2:虚拟 −1、真书 {0,+1};N=3:虚拟 −1,−2、真书 {0,+1,+2}。−half 恰是 mod 重复位,跳过即去重,真书无重复。
...
- **N≤3 有虚拟空位**:N=1 两侧空;N=2 左缘 1 空;N=3 左半 2 空。视觉数量 = N(一致),布局非对称(偶数真书无法以 slot0 对称;N=3 正侧占用使负侧空)。
...
- **N≥4 零变化**:原 reflow/virtual 不动,零回归。
- **虚拟位是 layer2 视觉概念**:不进 layer1 `/api/pages`,只在 `BookShelf3D` 内部凑 slots。

## 验证

- `computeShelfSlots` 测试:N=1,2 -> slots=3;N=3 -> slots=5;N=4/18 不变(锚点)。
- `computeRealSlots` 测试:N=3 slots=5 -> [0,1,2](正侧顺序对)。
- currentSlot 量化测试:virtual=false 永落 realSlots,virtual=true 自由。
- N=2 渲染 2 本、N=1 渲染 1 本、N=3 渲染 3 本(虚拟空位不渲染)。
- 现有 `bookShelfReflow`/`bookShelfInteraction` 测试不破(N≥4 路径不变)。
