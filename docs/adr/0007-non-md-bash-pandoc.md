...
选 bash+pandoc:pandoc 覆盖广(office 三件套 + odt/epub/html/rtf/csv/json/xml/org/rst 等,`--from=` 原生支持 docx/xlsx/pptx,无 experimental 标注);复用 pi bash 不写 customTool;白名单限定 bash 只跑 pandoc,化解 D6 的 rm -rf 顾虑。
...
- 只放行 `pandoc` 开头的单条命令(agent read 工具已能读纯文本,无需 cat)
- 禁止 shell 元字符(`;` `&&` `||` `|` `$()` `` ` `` `>` `<`),防止 `pandoc x; rm -rf` 绕过
- 非白名单 → `{ block: true, reason }`

bash 风险面从"任意命令"收窄到"pandoc 白名单",化解 D6 决策 1 的 rm -rf 顾虑。agent 工具集里只有 bash 能跑命令,管住 bash 即管住命令执行面(`read` 读文件不危险,`write`/`edit` 撞 raw/ 已被 kbHooks 拦,`grep`/`find`/`ls` 只读)。
