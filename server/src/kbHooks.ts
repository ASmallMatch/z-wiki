// 知识库钩子:UserPromptSubmit → input 事件,未命中"外部知识"关键词时 transform 注入知识库模式引导语
// (迁移自 share_wsl/.claude/settings.json 的钩子;同批迁移的 Stop → agent_end git 检测钩子已删——
// kb/ gitignored / 桌面版非 git 仓库,该检测恒为空转,变更检测走 buildView + hasIndexChanged)。
//
// 通过 DefaultResourceLoader({ extensionFactories }) 内联注入,无需独立 .pi/extensions 文件。
import path from 'node:path'
import type { ExtensionFactory } from '@earendil-works/pi-coding-agent'
import { isRawPath, isWithinKb, isWritablePath } from './kbLayout.js'
