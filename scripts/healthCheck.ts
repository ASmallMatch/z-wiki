...
async function main() {
  ...
  lines.push(
    `| 断链扫描 | ${broken.length === 0 ? '✅ 无断链' : `⚠️ ${broken.length} 条`} | ${broken.length ? '见下表' : '—'} |`,
  )
  ...
  const orphans = wikiFiles.filter(
    (w) => !referencedStems.has(w.stem.toLowerCase()) && w.stem !== '00-知识库导航',
  )
  lines.push(
    `| 孤儿 wiki | ${orphans.length === 0 ? '✅ 无孤儿' : `⚠️ ${orphans.length} 篇`} | ${orphans.length ? orphans.map((o) => o.stem).join(', ') : '—'} |`,
  )

  // 3. 空文件
  const empties = files.filter((f) => isEmptyOrOnlyFrontmatter(f.content))
  lines.push(
    `| 空文件 | ${empties.length === 0 ? '✅ 无' : `⚠️ ${empties.length} 个`} | ${empties.length ? empties.map((e) => e.rel).join(', ') : '—'} |`,
  )
  ...
  lines.push(
    `| 重复文件名 | ${dups.length === 0 ? '✅ 无' : `⚠️ ${dups.length} 组`} | ${dups.length ? dups.map(([s, arr]) => `${s}(${arr.length})`).join(', ') : '—'} |`,
  )
  ...
  lines.push(
    `| Frontmatter 覆盖率 | ${pct >= 80 ? '✅' : '⚠️'} ${pct}% | ${withFm}/${files.length} |`,
  )
  ...
  console.log(
    `  断链: ${broken.length}  孤儿: ${orphans.length}  空文件: ${empties.length}  重复: ${dups.length}  frontmatter: ${pct}%`,
  )
}
